<?php session_start();
	...
	if(isset($_POST['btnSignup'])){
		...
		$lms_r = lms_register($idno, $fname, $lname, $mi, $un, $pw,$conn);
	...
	if(isset($_POST['btnplogin'])){
		...
		$au = authParent($un, $pw,$conn);
		$pr = mysqli_fetch_object($au);
		if(mysqli_num_rows($au)){
	...
	if(isset($_POST['btnlogin'])){
		...
		$auth = authMember($uname,$pw,$conn);
		//$arr = mysql_fetch_array($auth);
		$count =mysqli_num_rows($auth);

		if($count!=0){
			
			$g=mysqli_fetch_object($auth);
	...
	if(isset($_POST['btnCreate'])){
		...
		$c = createClass($code, $course, $cdesc,$id_no,$conn);
		if($c==true){
	...
	if(isset($_POST['btnsave_module'])){
		...
		function getExtension($str) {
			...
			$do = creatModule($gid,$module,$mod_desc,$final_filename,$conn);
			if($do==1){
			...
			$action = "Added";
			$feed = createFeed($ftitle,$link,$desc,$gid,$action,$conn);
	...
	if(isset($_POST['btnSaveActivity'])){
		...
		$mod_id = $_GET['m'];
		$gid = $_GET['c'];
		$doInsert = creatExercise($mod_id, $name, $desc,$conn);
		if($doInsert!=0){
	...
	if(isset($_POST['btnUpdateActivity'])){
		...
		$name = sanitize($_POST['txtactname']);
		$desc = sanitize($_POST['txtactdesc']);
		$u = updateActivity($actid, $name,$desc,$conn);
		if($u==true){
	...
	if(isset($_POST['btnJoinClass'])){
		...
		$j = joinMember($stud_no, $lms_c_code,$conn);
	...
	if(isset($_POST['btn-importClass'])){
		...
		$c = icreateClass($nfilename[0], $nfilename[1], $nfilename[2],$nyear,$nfilename[4],$_SESSION['lms_m_id_no'],$conn);
		if($c!="exist"){
			...
			while (($data = fgetcsv($handle, 100000, ",")) !== FALSE) {
					...
					$i=true;
					if($chk==0){
						$i = lms_register($data[1], $ns[1], $ns[0],"", $data[1], $data[1],$conn);
					}else{
						$i=true;
					}
					$join = joinImport($data[1],$c,$conn);
					if($i!=true){
						$flag=1;
					}
					// echo $ns[0];
	...
	if(isset($_POST['btnUpdateAss'])){
		...
		// $n = newAssignment($c,$ass_t,$ass_d,$due,$id_no,"Active",$final_filename);
		$n = upAss($c,$ass_t,$ass_d,$due,$id_no,"Active",$final_filename,$assid,$conn);
	...
	if(isset($_POST["btnSubmitAss"])){
		...
		function getExtension($str) {
			...
			$i = saveSubmision($a,$idno,$final_filename,$conn);
			if($i==true){
			...
			$action = "Submit";
			$feed = createFeed($ftitle,$link,$desc,$c,$action,$conn);
			header("Location: dashboard.php?group=$c&t=assignments");
	}
	...
	if(isset($_POST['btnCreateQuiz'])){
		...
		$create = create_Quiz($idno, $class_id,$quiz_title,$limit,$duedate,$conn);
	...
	if(isset($_POST['btnSaveQuestion'])){
		...
		$suc = newQuestion($qid,$type,$op,$question,$ans,$conn);
		if($suc==true){
	...
	if(isset($_POST['btnUpdateQuestion'])){
		...
		$suc = updateQuestions($qid,$ques_id,$type,$choice,$question,$ans,$conn);
		if($suc==true){
	...
	if(isset($_POST['btnSubmitAns'])){
		...
		// echo "<br />";
		// print_r($answer);
		$save = saveAnser($quesid,$idno,$answer,$qid,$conn);
	...
	if(isset($_POST['btnShare'])){
		...
		// print_r($st);
		foreach ($st as $val) {
			$ishare = shareIt($oid, $val, $mid,$conn);
			if($ishare==true){
				$eflag = 0;
			}else{
				$eflag=1;
	...
	if(isset($_POST['btnChange'])){
		...
		if(chkPassword($pw, $user,$conn)==true){
			$npw = sanitize($_POST['txtnpass']);
			$cpw = sanitize($_POST['txtcpass']);	

			if($npw==$cpw){
				$dc = changePass($cpw, $user,$conn);
				//echo $dc;
				if($dc==true){
	...
	if(isset($_POST['btnNguard'])){
		...
		$add = addGuardians($fname, $lname, $mname, $_SESSION['lms_m_id_no'],$conn);
		if($add==true){
	...
	if(isset($_POST['btnUguard'])){
		...
		$gu = updateGuardian($fname, $lname, $mname, $id,$conn);
		if($gu==true){
